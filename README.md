...
<div align="center">
  ...
  <p align="center">
    This paper has been submitted for publication in Elsevier Journal, Neural Computing and Applications.
    <br />
   </p>
...
<p align="justify"> Speech emotion recognition (SER) poses a considerable challenge due to the inherent ambiguity in human emotions, rendering their identification a complex task. In this study, we present CNN-n-GRU, an innovative end-to-end (E2E) deep learning model tailored for SER. The proposed architecture comprises an \(n\)-layer convolutional neural network (CNN) succeeded by an \(n\)-layer gated recurrent unit (GRU). The motivation behind this architecture stems from the promising outcomes observed in both CNNs and recurrent neural networks (RNNs) when processing raw waveform speech input. This impetus led to the integration of CNNs and RNNs into a unified framework, capitalizing on the strengths of both approaches. Leveraging the capability of CNN models to generate multiple abstraction levels for feature representation, we employ the CNN as the initial component to extract high-level features. These features are subsequently fed into RNN layers for the aggregation of long-term dependencies. Notably, our training methodology emphasizes enabling the CNN component to discern high-level speech representations directly from raw waveform data, eliminating the need for handcrafted features or spectrograms. This strategy enhances the network's capacity to capture pertinent narrow-band emotion features while adeptly handling variable-length speech without necessitating segmentation. Simultaneously, the GRU component excels in learning temporal-based characteristics, facilitating the model's adept capture of time-distributed features within the signal. The efficacy of the proposed CNN-n-GRU model is rigorously assessed for SER through comparisons with state-of-the-art methodologies on TESS and IEMOCAP datasets. Our experimental results underscore the superior performance achieved by the CNN-n-GRU model, thereby outperforming contemporary SER approaches. </p>
<div align="center">
