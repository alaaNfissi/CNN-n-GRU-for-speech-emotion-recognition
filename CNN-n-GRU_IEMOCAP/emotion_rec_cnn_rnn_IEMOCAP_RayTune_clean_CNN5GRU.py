...
iemocap_folder = '/home/alaa/Downloads/Concordia/TÉLUQ/ser/Explo_1/Ph.D/CNN-n-GRU-for-speech-emotion-recognition'
iemocap_experiments_folder = '/home/alaa/Downloads/Concordia/TÉLUQ/ser/Explo_1/Ph.D/CNN-n-GRU-for-speech-emotion-recognition/CNN-n-GRU_IEMOCAP/iemocap_experiments'
...
def train_CNN5GRU(config, checkpoint_dir=None, data_path=None, max_num_epochs=None):
    ...
    model_CNN5GRU = CNN5GRU(n_input=config["n_input"], hidden_dim=config["hidden_dim"], n_layers=config["n_layers"] , n_output=config["n_output"])
    ...
    if torch.cuda.is_available():
        ...
        if torch.cuda.device_count() > 1:
            model_CNN5GRU = nn.DataParallel(model_CNN5GRU)
    model_CNN5GRU.to(device) 
    
    optimizer = optim.Adam(model_CNN5GRU.parameters(), lr=config["lr"], weight_decay=config["weight_decay"])
    lr_scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=20, gamma=0.1)
    
    if checkpoint_dir:
        model_state, optimizer_state = torch.load(
            os.path.join(checkpoint_dir, "checkpoint"))
        model_CNN5GRU.load_state_dict(model_state)
    ...
    for epoch in range(1, epoch_count + 1):
        ...
        model_CNN5GRU.train()
        right = 0
        h = model_CNN5GRU.init_hidden(int(config["batch_size"]), device)
        ...
        with tune.checkpoint_dir(epoch) as checkpoint_dir:
            ...
            torch.save((model_CNN5GRU.state_dict(), optimizer.state_dict()), path)
...
def main(num_samples=10, max_num_epochs=10, gpus_per_trial=1):
    data_path = os.path.abspath('../Data_exploration/IEMOCAP_dataset.csv')
    train_set, validation_set, _ = init_data_sets(load_data(data_path))
    ...
    result = tune.run(
        tune.with_parameters(train_CNN5GRU, data_path=data_path, max_num_epochs=max_num_epochs),
        resources_per_trial={"cpu": 32, "gpu": gpus_per_trial},
        config=config,
        num_samples=num_samples,
        scheduler=scheduler,
        progress_reporter=reporter,
        local_dir=os.path.abspath(iemocap_experiments_folder+"/IEMOCAP_CNN_5_GRU"),
        log_to_file=(os.path.abspath(iemocap_experiments_folder+"/IEMOCAP_CNN_5_GRU_stdout.log"), os.path.abspath(iemocap_experiments_folder+"/IEMOCAP_CNN_5_GRU_stderr.log")),
        name="IEMOCAP_CNN_5_GRU",
        resume='AUTO')
    ...
    best_trained_model = CNN5GRU(n_input=best_trial.config["n_input"], hidden_dim=best_trial.config["hidden_dim"], n_layers=best_trial.config["n_layers"] , n_output=best_trial.config["n_output"])
    device = "cpu"
    if torch.cuda.is_available():
        device = "cuda:0"
        if gpus_per_trial > 1:
    ...
    CNN5GRU_test_acc_result, y_pred, y_true = test(best_trained_model, best_trial.config["batch_size"], data_path)
    #test_acc = test_accuracy(best_trained_model, device)
    print("Best trial test set accuracy: {}".format(CNN5GRU_test_acc_result))
